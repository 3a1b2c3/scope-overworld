...
Usage (from scope-overworld root):
    python src/scope_overworld/vbench.py [--output_dir output/vbench/videos] [--num_samples 5] [--num_frames 64] [--fps 16]
"""
import argparse
import csv
import json
...
def generate_video(pipeline, image_path: str, caption: str, num_frames: int) -> torch.Tensor:
    ...
    result = pipeline(images=[image_path], prompts=[caption], init_cache=True)
...
def main():
    ...
    if stats_is_new:
        stats_w.writerow(["timestamp", "task_idx", "prompt", "sample_idx", "seed", "duration_s",
                          "gen_fps", "video_count", "total_elapsed_s", "avg_s_per_video",
    ...
    for task_idx, (image_name, caption) in enumerate(prompts):
        ...
        for sample_idx in range(args.num_samples):
            ...
            try:
                ...
                print(f"[vbench]   duration {_fmt_duration(duration)}  |  {gen_fps:.2f} gen-fps  |  avg {avg_s_per/60:.1f} min/video")
